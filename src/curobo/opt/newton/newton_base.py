...
class NewtonOptBase(Optimizer, NewtonOptConfig):
    ...
    def _optimize(self, ee: str, q: T_BHDOF_float, shift_steps=0, n_iters=None):
        ...
        if not self.cu_opt_init:
            self._initialize_opt_iters_graph(ee, q, grad_q, shift_steps=shift_steps)
        for i in range(self.outer_iters):
            best_q, best_cost, q, grad_q = self._call_opt_iters_graph(ee, q, grad_q)
            if (
    ...
    def _opt_iters(self, ee: str, q, grad_q, shift_steps=0):
        q = q.detach()  # .clone()
        grad_q = grad_q.detach()  # .clone()
        for _ in range(self.inner_iters):
            self.i += 1
            cost_n, q, grad_q = self._opt_step(ee, q.detach(), grad_q.detach())
        if self.store_debug:
    ...
    def _opt_step(self, ee:str, q, grad_q):
        with profiler.record_function("newton/line_search"):
            q_n, cost_n, grad_q_n = self._approx_line_search(ee, q, grad_q)
    ...
    def _compute_cost_gradient(self, ee: str, x):
        x_n = x.detach().requires_grad_(True)
        x_in = x_n.view(
            self.n_problems * self.num_particles, self.action_horizon, self.rollout_fn.d_action
        )
        trajectories = self.rollout_fn(ee, x_in)  # x_n = (batch*line_search_scale) x horizon x d_action
    ...
    def _wolfe_line_search(self, ee: str, x, step_direction):
        ...
        b, h, _ = x_set.shape
        c, g_x = self._compute_cost_gradient(ee, x_set)
        with torch.no_grad():
    ...
    def _greedy_line_search(self, ee: str, x, step_direction):
        step_direction = step_direction.detach()
    ...
    def _armijo_line_search(self, ee: str, x, step_direction):
        step_direction = step_direction.detach()
        step_vec = step_direction.unsqueeze(-2)
        x_set = x.unsqueeze(-2) + self.alpha_list * step_vec
        x_set = self.clip_bounds(x_set)
    ...
    def _approx_line_search(self, ee: str, x, step_direction):
        ...
        if self.line_search_type == LineSearchType.GREEDY:
            best_x, best_c, best_grad = self._greedy_line_search(ee, x, step_direction)
        elif self.line_search_type == LineSearchType.ARMIJO:
            best_x, best_c, best_grad = self._armijo_line_search(ee, x, step_direction)
        elif self.line_search_type in [
            LineSearchType.WOLFE,
            LineSearchType.STRONG_WOLFE,
            LineSearchType.APPROX_WOLFE,
        ]:
            best_x, best_c, best_grad = self._wolfe_line_search(ee, x, step_direction)
        if self.fix_terminal_action and self.action_horizon > 1:
    ...
    def _initialize_opt_iters_graph(self, ee: str, q, grad_q, shift_steps):
        if self.use_cuda_graph:
            self._create_opt_iters_graph(ee, q, grad_q, shift_steps)
    ...
    def _call_opt_iters_graph(self, ee: str, q, grad_q):
        if self.use_cuda_graph:
            ...
            return self._opt_iters(ee, q, grad_q)

    def _create_opt_iters_graph(self, ee: str, q, grad_q, shift_steps):
        # create a new stream:
        self.reset()
        ...
        with torch.cuda.stream(s):
            for _ in range(3):
                self._cu_opt_q, self._cu_opt_cost, self._cu_q, self._cu_gq = self._opt_iters(ee,
